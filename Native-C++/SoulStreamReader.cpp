...
void SoulStreamReader::startStream(){
	...
	while(getline(lines, line)){
		...
		bool infmatch = srell::regex_search(templine, m_SoulParser->preProMatch);
		if(infmatch){
	...
	while(getline(varlines, varline)){
		...
		std::string templine = varline;
		bool infmatch = srell::regex_search(templine, m_SoulParser->preProMatch);
		bool groupmatch = srell::regex_search(templine, m_SoulParser->groupMatch);
		bool groupvarmatch = srell::regex_search(templine, m_SoulParser->groupVarMatch);
		bool varmatch = srell::regex_search(templine, m_SoulParser->varMatch);
	...
	while(getline(grouplines, grpline)){
		...
		std::string templine = grpline;
		bool groupmatch = srell::regex_search(templine.c_str(), groupMatch, m_SoulParser->groupMatch);
		if(groupmatch){
			remove(templine, "Â");
	...
	while(getline(groupvarlines, grpVarline)){
		...
		std::string templine = grpVarline;
		bool groupvarmatch = srell::regex_search(templine.c_str(), groupVarMatch, m_SoulParser->groupVarMatch);
		if(groupvarmatch){
			remove(templine, "Â");
